...
export const fethcProductDetails = createAsyncThunk(
...
export const fetchProductWithAI = createAsyncThunk(
  ...
  async (userPrompt, thunkAPI) => {
    try {
      console.log("FRONTEND", userPrompt);
      const res = await axiosInstance.post(`/product/ai-search`, {userPrompt});
      thunkAPI.dispatch(toggleAIModal())
      return res.data;
    } catch (error) {
      toast.error(error.response?.data?.message || error.message || "AI search failed");
      throw thunkAPI.rejectWithValue(
        error.response?.data?.message ||
          error.message ||
          "Failed to fetch AI Filtered product",
      );
    }
  },
);
...
const productSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fethcProductDetails.pending, (state) => {
        state.loading = true;
      })
      .addCase(fethcProductDetails.fulfilled, (state, action) => {
        state.loading = false;
        state.productDetails = action.payload;
        state.productReviews = action.payload.reviews;
      })
      .addCase(fethcProductDetails.rejected, (state, action) => {
        state.loading = false;
      })
          ...
          state.productReviews = [
            {
              ...
              reviewer: {
                ...
                avatar: state.authUser?.avatar?.url
              },
            },
            ...state.productReviews,
          ]
